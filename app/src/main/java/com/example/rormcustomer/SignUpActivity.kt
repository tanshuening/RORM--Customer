...
import com.example.rormcustomer.databinding.ActivitySignUpBinding
...
class SignUpActivity : AppCompatActivity() {
    ...
    private fun createAccount(email: String, password: String) {
        auth.createUserWithEmailAndPassword(email, password).addOnCompleteListener { task ->
            if (task.isSuccessful) {
                ...
                Toast.makeText(this, "Failed to create account: $errorMessage", Toast.LENGTH_SHORT)
                    .show()
    ...
    private fun checkIfUserExists(userId: String, user: User) {
        database.child("users").child(userId).get().addOnSuccessListener { snapshot ->
            if (snapshot.exists()) {
                // User already exists, proceed to LoginActivity
                val intent = Intent(this, LoginActivity::class.java)
                startActivity(intent)
                finish()
            } else {
                ...
                database.child("users").child(userId).setValue(user).addOnCompleteListener { task ->
                    if (task.isSuccessful) {
                        ...
                        Log.e(
                            "SignUpActivity",
                            "saveUserData: Failure: $errorMessage",
                            task.exception
                        )
                    }
                }
            ...
            Log.e("SignUpActivity", "Error checking user existence", exception)
        }
    }
}
